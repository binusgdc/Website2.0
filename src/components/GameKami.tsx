...
export default function GameKami({ gameData }) {
...
export async function getStaticProps() {
    const gameData = await fetch(`https://itch.io/api/1/${process.env.ITCH_KEY}/my-games`, {
        method: "GET",
    }).then((response) => {
        if (!response.ok) {
            throw new Error("Cannot connect because: " + response.statusText)
        }
