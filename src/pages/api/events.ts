...
function checkRequestHeader(r: PackedRequestData, contentType: ContentType) {
	...
	if (requestContentType !== contentType) {
		console.log(`[${client}] sent "${requestContentType}", but we are expecting Content-Type "${contentType}"`)
		respond(r, 415)
		return false
	}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	const resp = ((code: number) => { respond(r, code) })
	...
	switch (req.method) {
		case "POST": {
			...
			if ((await Event.find({id: req.body.id}).lean()).length > 0) {
				console.log(`[${client}]'s request to POST id ${req.body.id} cannot be fulfilled because it already exists`)
				resp(409)
				return
			}
		...
		case "DELETE": {
			...
			if ((~~deletedCount) <= 0) {
				console.log(`[${client}]'s request to DELETE id ${req.body.id} failed because it does not exist`)
				resp(404)
				return
			}
