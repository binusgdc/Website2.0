...
export default function Server({ time }: ServerProps) {
    const [timeNow, setTimeNow] = React.useState<string>(time);
    ...
    return (
        <div>
            ...
            <p>Contoh server time di bawah ketika page ini fetch ke <code>/api/test</code> pakai <code>getServerSideProps()</code></p>

            <p><b>Server time saat page load: </b>{time}</p>


            <p>Ini baru real time pakai <code>useEffect()</code></p>
            <p><b>Real time: </b>{timeNow}</p>
        </div>
    )
}
